...
myForm.onsubmit = (e) => {
    ...
    const product = {
            title, description, code, price, stock, category, thumbnail, operation: 'add'
    };
...
edit_form.onsubmit = (e) => {
    e.preventDefault();
    const input_vals =  Array.from(document.querySelectorAll('#edit_form input'));
    const obj = {
        id: parseInt(input_vals[0].value),
        title: input_vals[1].value,
...
btnDeletes.forEach(b_delete => {
    b_delete.addEventListener('click', (e) => {
        e.preventDefault();
        let obj = {
            id: parseInt(e.target.closest('td').className),
            operation: 'delete'
        }
        socket.emit('operation', obj);
    });
